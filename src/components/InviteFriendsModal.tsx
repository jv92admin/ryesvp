  ...
  return (
    <Dialog open={true} onOpenChange={() => onClose()} size="md">
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {friends.map((friend) => (
              <div
                key={friend.id}
                className="flex items-center gap-3 p-3 bg-[var(--bg-surface)] rounded-lg"
              >
                <div
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-[var(--lark-text-muted)]">{friend.email}</p>
                </div>
                {invited.has(friend.id) ? (
                  <span className="text-sm text-[var(--lark-text-primary)] font-medium">✓ Invited</span>
                ) : (
                  <button
                    onClick={() => handleInvite(friend.id)}
                    disabled={inviting.has(friend.id)}
                    className="px-3 py-1.5 text-sm font-medium rounded-lg transition-colors bg-[var(--accent)] text-white hover:bg-[var(--accent)] disabled:opacity-50"
                  >
      ...
      <DialogFooter>
        <button
          onClick={onClose}
          className="w-full px-4 py-2 text-sm font-medium text-[var(--lark-text-secondary)] bg-[var(--bg-surface)] rounded-lg hover:bg-[var(--bg-hover)] transition-colors"
        >
          Done
        </button>
      </DialogFooter>
